document.addEventListener('DOMContentLoaded', function () {
    ...
    function checkValidityCustom(formElement, tooltipIdText) {
        ...
        for (let i = 0; i < requiredInputs.length; i++) {
            if (requiredInputs[i].type === 'file') {
                if (requiredInputs[i].files.length === 0 && (!preview || !preview.src || preview.src === window.location.href)) {
                    isValid = false;
    ...
    if (btnBack) {
        btnBack.addEventListener('click', function (e) {
            if (btnSimpan && btnSimpan.style.display === 'block' && btnBukaEdit.style.display === 'none') {
                e.preventDefault();
                formEdit.reset();
                aktifkanModeView();
                ...
                if (preview) {
                    ...
                    if (!previewOriginalSrc || previewOriginalSrc === window.location.href) {
                        preview.style.display = 'none';
                        if (buntelanAwal) buntelanAwal.style.display = 'flex';
                    } else {
                        preview.style.display = 'block';
                        if (buntelanAwal) buntelanAwal.style.display = 'none';
                    }
                }
    ...
    // ==========================================
    // 12. EVENT LISTENER UPLOAD FOTO (GALERI & KAMERA)
    // ==========================================
    const btnPilihGaleri = document.getElementById('btnPilihGaleri');
    const btnAmbilFoto = document.getElementById('btnAmbilFoto');
    const fotoUploadUtama = document.getElementById('foto_upload_utama');

    if(btnPilihGaleri && fotoUploadUtama) {
        btnPilihGaleri.addEventListener('click', function() {
            fotoUploadUtama.removeAttribute('capture');
            fotoUploadUtama.click();
        });
    }

    if(btnAmbilFoto && fotoUploadUtama) {
        btnAmbilFoto.addEventListener('click', function() {
            fotoUploadUtama.setAttribute('capture', 'environment');
            fotoUploadUtama.click();
        });
    }

    // Global fallback biar atribut onclick di HTML kalau kepakai tetep jalan
